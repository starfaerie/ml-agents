...
public class FoodCollectorArea : Area
{
    ...
    public void ResetFoodArea(GameObject[] agents, GameObject[] ghosts)
    {
        ...
        {
            if (ghost.transform.parent == gameObject.transform)
            {
                ghost.transform.position = new Vector3(Random.Range(-range, range), 3.5f,
                                               Random.Range(-range, range))
                                           + transform.position;
                ghost.transform.rotation = Quaternion.Euler(new Vector3(0f, Random.Range(0, 360)));
            }
        }
